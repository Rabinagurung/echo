...
const CustomizationForm = ({
  ...
  const onSubmit = async (data: FormSchema) => {
    try {
      ...
      // Parse allowed domains from the form — filter out empty entries
      ...
      <form className="space-y-6" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Control which websites can embed your chat widget. Only listed
              domains will be able to load the widget. For local development,
              use the full origin with port (e.g. http://localhost:3002).
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
              ...
              render={({ field }) => {
                ...
                return (
                  <FormItem>
                    ...
                    <FormDescription>
                      Enter one domain per line. Use *.example.com for wildcard
                      subdomains. For non-standard ports, include the full
                      origin (e.g. http://localhost:3002).
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                );
            ...
            />
            {(!form.watch("allowedDomains") ||
              form.watch("allowedDomains")?.filter((d) => d.trim()).length ===
                0) && (
              <p className="text-sm text-yellow-600">
                No domains configured — your widget will only be accessible
                directly, not embeddable on any external site.
              </p>
            )}
          </CardContent>
        </Card>
